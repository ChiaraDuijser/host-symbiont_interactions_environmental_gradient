---
title: "Coral Reefs - Symbiodiniaceae diversity"
...
```{r user_definitions}
setwd("../Symbiodiniaceae diversity/")
```
...
Metadata now includes an extra column *population* which contains the populations based on STRUCTURE and DAPC analysis. Clones are included for symbiont analysis as they can harbour different symbionts.

```{r}
# Import sequence table
seqs <- read_tsv("../Symbiodiniaceae diversity/20230421T034816_SymPortal/post_med_seqs/259_20230421T204816_DBV_20230422T042656.seqs.absolute.abund_and_meta.txt") %>%
    mutate(sample_name = as.factor(sample_name)) %>%
    filter(!(is.na(sample_name))) # remove the final empty row;
...
meta <- read.csv("../Symbiodiniaceae diversity/230423 metadata_symbiont_diversity.csv", sep=",")
...
profiles_raw <- read_tsv("../Symbiodiniaceae diversity/20230421T034816_SymPortal/its2_type_profiles/259_20230421T204816_DBV_20230422T042656.profiles.absolute.abund_and_meta.txt", skip = 6) %>% #first 6 rows is other text
...
#ggsave("profiles_pie_HYBRID_cloneincl2.pdf")
```
...
all_pal <- readRDS("../Symbiodiniaceae diversity/color_palette.rds")
...
fasta <- read_fasta_df("../Symbiodiniaceae diversity/20230421T034816_SymPortal/post_med_seqs/259_20230421T204816_DBV_20230422T042656.seqs.fasta") %>%
...
# PERMANOVA of metadata factors (population and colony_position) and their explaining power of the unifrac distance
```{r}
...
# the below formula tests for an effect of population on the ITS2 unifrac distance, and then tests for an effect of colony_position, nested within each population
library(vegan)
...
# call the permanova test and specify the model factors
adonis2(dist ~ population, data = adonis_meta2)
adonis2(formula = dist ~ population, data = adonis_meta2)

###### POPULATION Clones included #########
#           Df SumOfSqs      R2      F Pr(>F)    
#population  2  18.2310 0.75952 146.87  0.001 ***
#Residual   93   5.7722 0.24048                  
...
This will be a PCoA of the generalized unifrac distance with alpha 0.5 on which PERMANOVA was performed.
...
##Total percentage within each of the four habitats that Durusdinium sequences make up in the data. 
...
##Total percentage within each of the four habitats that Cladocopium sequences make up in the data.
...
##Total percentage within each of the four habitats that Breviolum sequences make up in the data.
...
##Total percentage within each of the four habitats that Fugacium sequences make up in the data.
...
### Supplementary Figure 9 (adjusted in Adobe Illustrator)
...
# Save the plots
#ggsave("back_Reef_colpos_cloneincl_2.0.pdf", plot = back_Reef_colpos)
#ggsave("back_Mangrove_colpos_cloneincl_2.0.pdf", plot = back_Mangrove_colpos)
#ggsave("back_Hybrid_colpos_cloneincl_2.0.pdf", plot = back_Hybrid_colpos)
```
