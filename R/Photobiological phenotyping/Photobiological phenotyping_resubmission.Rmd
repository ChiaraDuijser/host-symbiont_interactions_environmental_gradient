...
The red boxes indicate the estimate of Ek and confidence interval of the parameters

Model fit = Hennige 2008 equation. E on x-axis is PAR.

Ek prediction for curve_id 18 (sample 7; mangrove, cryptic) and 19 (sample 8; mangrove, cryptic) lies beyond the actual data points, which is weird. The Hennige model is different because an exponential decay in FqFm is expected, but we see already see a sharp drop from 0.1 to 10 PAR (first light step). This is why the model predictions/fitted values and observed values don't align.

Curve_id 12 (sample 131; reef, cryptic) is most like we would expect it to be. It was a thin red line which means small 95% CI around the predicted Ek value based on the Hennige model and our data.
...
Start for the params measured under darkness

carQ = carotenoid quencing; develops under condition of high excitation power, typical of the saturation portion of the QA fluorescence transient

Ek = saturating irradiance; defined as point at which the extrapolated initial slope crosses Pmax => Ek represents an optimum on the photosynthesis irradiance curve. Higher Ek values indicate acclimation to high irradiance (as Ek is presented in units of PAR). Ek is higher for reef samples, which would be expected because reef corals are in less turbid waters so higher light intensities. So they are able to photosynthesize more under high light intensities.

FqFm = measurements of quantum yield under light. Values appear higher for exposed fragments. Able to photosynthesize more (or more efficiently?) under specific light conditions compared to cryptic symbionts.

p = probability of energy transfer; describes a probablity that excitation energy can be shared between reaction centers. If p = 0 there's no energy transfer (first-order saturation), does this mean that all energy is lost to heat/non-photochemical quenching (NPQ)?. Here, p = 0.2 (set value).

PQP_size = plastoquinone pool size; PQ pool = mobile pool of plastoquinone within thylakoid bilyaer that mediates electron transport between QB component of PSII and cyt bf complex by a mechanisms of "long distance" electron shuttle. PQ pool gets oxidized (i.e., it loses electrons). Does PQ reoxidation mean reduction (getting electrons) of PQ pool so that it can be oxidized again during next light pulse?

QBP_size = QB pool size

Sig = functional absorption cross section of PSII; defines the effieciency of photosynthetic light utilization (EPU) by PSII. If this defines the efficiency of photosynthetic light utilization, is this values then the mean value of FqFm because that resembles photosyntehtic efficiency of PSII? => Yes, sigma and Fv/Fm can be correlated (see Suggett et al., 2015 New Phytologist). It can also be correlated with Ek (see Suggett et al., 2022 Limnology and Oceanography methods).

Tau1QA; min. electron turnover time from QA to QB => comes from relaxation phase
Tau2QA; min. electron turnover time from Qb to PQ => comes from relaxation phase
Tau3QA; min. electron turnover time from PQ to PSI => comes from relaxation phase
TPQ_PSI; ?
...
Look at PCA plot for each  different light step and label the individual points with text label within code. Maybe check for outliers.

Principal Component Analysis clears the "noise" to allow you to identify patterns in the data (removes redundancy). It reduces dimensions by projecting the whole data set onto a completely separate space without losing too much information.

The first principal component accounts for the largest possible variances (variances = what measures the amount of information). The vector will point into the direction of the larger spread of data (Eigenvector), the number will be equal to the spread (variance) of that direction (Eigenvalue).
...
PCA biplot = merge an usual PCA plot with a plot of loadings
...
Since mangrove and reef are really distinct at p_0.1 this probably indicates that they’re “enough” low light acclimated for the Soliense to work. Making a distinction between cryptic and exposed is hard because you don’t know the exact light intensities they’re receiving. Sand can also reflect light. To really know differences between parts of 1 colony you would have to sample a whole colony and look at different parts of it.
(Graph going down/up/down is probably artefact of Soliense. Glitch in one of the first light steps)

So I will test for differences between mangrove (site 1) and reef (site 4) for light intensity 0.1 with a PCA plot. First, however it's important to check for outliers in dataset (looking at data, curve_id 18 and 19 could be outliers) and remove if needed. Outlier removal has to be done before data normalization otherwise you may end up with different variables having different standard deviations (which is an issue, for example, in PCA analysis; https://stats.stackexchange.com/questions/415704/should-you-standardize-your-variables-before-or-after-removing-outliers). 

# Check for outliers and remove if needed to make PCA plot + do statistics on this data
I would have to make a PCA plot with outliers removed if they should be removed according to R code/function. And then do statistics on PCA ordination/loadings extracted from the plot.

First, create a dataset from the dark_params, i.e. parameters generated by the Soliense at PAR_level = 0.1.
...
View(dark_params) #20 observations with parameters generated by Soliense for PAR_level = 0.1
...
dark_params_long %>%
  doo(~mahalanobis_distance(.)) %>% #long-form does work; 2, 14, 18 (with SigLCHII 2, 14, 18 and 19)
  filter(is.outlier==T) # == instead of = because it's a logical

dark_params_long %>%
  doo(~mahalanobis_distance(dark_params_long, value)) %>% #14, 18, 19 => value is parameter of interest
  filter(is.outlier==T) 

# Check why without grouping by params? Mahalanobis Distance works well when two or more variables are highly correlated and even if their scales are not the same, because MD uses a covariance matrix (unlike Euclidean Distance). mahalanobis_distance(data, ...) -> ... used to select a variable of interest; below value is our variable of interest and we compute distance by groups, i.e. here params.
dark_params_long %>%
  group_by(params) %>%
  doo(~mahalanobis_distance(dark_params_long, value)) %>% #added value changes things; 14, 18 and 19 are outliers
  filter(is.outlier==T) 

##grouping doesn't work, so I could do it manually for each level of params separately (that's what Amanda did)
...
#dark_params_NOut$population <- factor(dark_params_NOut$population, labels=c("Population 2", "Hybrid"))
str(dark_params_NOut)
...
# I have to remove p due to constant number which will cause trouble later on if included
...
https://rpubs.com/oldriku/1090218

adonis just like regular MANOVA or ANOVA gives an overall significance if any of the clusters are significantly different. One would still need to do some sort of post-hoc/pairwise test to verify significance between different clusters. I wonder if there is a non-parametric version of the Hotelling's T2 test.
...
The below will give you Dim.1, Dim.2, Dim.3, etc. values - these are basically PC1, PC2, etc. - I only looked at the Dim.1 and Dim.2 values. Results for variables will give you results for the values of the eigenvectors (lines in your PCA plot) - e.g. for me Soliense parameters.

I used Contributions of variables to the PCs, to see which Soliense parameters contributed the largest loadings to PC1 and PC2 (no stats needed for this - just look at values from running 'res.var$contrib').

Results for individuals will give you results for each replicate (this is what I ran stat analysis on - specifically 'res.ind$coord' - from this analysis this told me whether site (mangrove or reef) separated out from each other. If you get significance this means the site that are significantly different separated out from one another. Because I ran Dim.1 and Dim.2 separately for stat analysis, if you get significance for Dim.1, means species were separated by PC1, if get significance for Dim.2, means sites were separated by PC2)
```{r}
LIFT_Statistics <- "~/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_working/LIFT-Data/R/"
...
PCAindcoor_DarkParams_site <- read_csv("~/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_working/LIFT-Data/R/PCAindcoor_DarkParams_SigLCHII_site_resubmission.csv")
...
# I will remove the clones from the dataset that is not yet normalized
