...
## To do
* Calculate average temp and pH per month for reef and mangrove => in table
* Calculate average temp and pH per day => perhaps better to plot due to so many data points


## Set working directory
```{r}
setwd("/Users/chiara/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023")
```


## Mangrove data
```{r}
m_log <- read.csv("/Users/chiara/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023/2022-2023 Mangrove HOBO_Temp_pH.csv", header=FALSE, comment.char="#", stringsAsFactors=TRUE)
...
library(lubridate)
#m_log$Date_Time <- as.POSIXct(m_log$Date_Time, format = "%Y-%m-%d %H:%M:%S") 
m_log$Date_Time <- as.POSIXct(m_log$Date_Time, format = "%d/%m/%Y %H:%M") 
str(m_log)

# Check for NA values in the dataset
anyNA(m_log)
anyNA(m_log$Date_Time)
which(is.na(m_log$Date_Time)) #2022-10-02 02:16:20 is missing for some reason
m_log$Date_Time[1184] <- as.POSIXct("2022-03-27 02:16:00")
anyNA(m_log) # no NAs anymore
...
Eventually we chose for 25 +/- 10 following the manufacturers statement.
...
103 datapoints are outside 25+/-10. This means that the pH value for 103 datapoints in the mangrove is not reliable. Remove those datapoints and make a note in the paper

## Remove 103 datapoints
```{r}
...
# If this works the for-loop should give no output! Check:
anyNA(m_subset_removed)

m_temp_table_removed <- data.frame()
for (i in 1:nrow(m_subset_removed)) {
  if (m_subset_removed$Temperature[i] >= 25 + 10 | m_subset_removed$Temperature[i] <= 25 - 10) {
    m_temp_table_removed <- rbind(m_temp_table_removed, m_subset_removed[i,])
  }
}

m_temp_table_removed # It worked!
```
We can see that there are no values that are outside 25 degrees +/-10, so we have removed the datapoints that give an unreliable pH value.
...
# Sort the data frame by pH values
m_subset_removed_sorted <- m_subset_removed[order(m_subset_removed$pH_T), ]

# Get the lowest 10 pH values and their corresponding date/time
lowest_10 <- head(m_subset_removed_sorted, n = 20)
...
No limits=lims needed as this is the subsetted dataset
...
## Reef data 
```{r}
r_log <- read.csv("/Users/chiara/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023/2022-2023 Reef HOBO_Temp_pH.csv", header=FALSE, comment.char="#", stringsAsFactors=TRUE)
...
# The time variable isn't at the date format, so time series data won't work. Use "lubridate" to convert it 
library(lubridate)
#r_log$Date_Time <- as.POSIXct(r_log$Date_Time, format = "%Y-%m-%d %H:%M:%S") 
...
r_log$Date_Time[1184] <- as.POSIXct("2022-03-27 02:23:00")
...
```{r}
m_log2 <- read.csv("/Users/chiara/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023/2022-2023 Mangrove 2nd logger.csv", header=FALSE, comment.char="#")
...
anyNA(m_log2)
anyNA(m_log2$Date_Time)
which(is.na(m_log2$Date_Time)) #NA for date_time 2022-10-02 02:23:51
m_log2$Date_Time[5722] <- as.POSIXct("2022-10-02 02:36:00")
anyNA(m_log2) # remaining NAs in beginning of dataset that will be filtered out below
...
### Reef 2
```{r}
r_log2 <- read.csv("/Users/chiara/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023/2022-2023 Reef 2nd logger.csv", header=FALSE, comment.char="#")
...
anyNA(r_log2)
anyNA(r_log2$Date_Time)
which(is.na(r_log2$Date_Time)) #NA for date_time 2022-10-02 02:23:51
r_log2$Date_Time[1186] <- as.POSIXct("2022-03-27 02:35:00")
anyNA(r_log2) # remaining NAs in beginning of dataset that will be filtered out below
...
#### m_subset for all observation outside calibration range as well ####
# Convert Date_Time to Date
m_subset$Date <- as.Date(m_subset$Date_Time)

# Calculate average temperature per day
m_all_average_temp_per_day <- aggregate(Temperature ~ Date, data = m_subset, FUN = mean)

# Print the result
print(m_all_average_temp_per_day)
write.csv(m_all_average_temp_per_day, file = "m_all_average_temp_per_day.csv")

mean(m_all_average_temp_per_day$Temperature, na.rm=TRUE)
max(m_all_average_temp_per_day$Temperature, na.rm=TRUE)
min(m_all_average_temp_per_day$Temperature, na.rm=TRUE)
```
...
### Two Sample t-test Daily averages

```{r}
daily_t <- read.csv("~/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023/m_r_average_temp_per_day.csv", stringsAsFactors=TRUE)
...
daily_max <- read.csv("/Users/chiara/Library/Mobile Documents/com~apple~CloudDocs/Documents/Studie/UvA Master/Research Project/Research Project 2 - UTS/Data_FINAL/Abiotic data/Feb 2022 - Feb 2023/Daily_max_M_R.csv", stringsAsFactors=TRUE)
str(daily_max)
